...
} from "./features/coding/manifest.js";
...
async function existingDirectory(path: string): Promise<boolean> {
  try {
    ...
    if (entry.isSymbolicLink() || !entry.isDirectory())
      throw new Error(
        "mtm-coding: skill root is not a regular directory: " + path,
      );
    return true;
  } catch (error) {
...
function remoteUrl(source: MtmCodingSkillSource, path: string): string {
  ...
  if (
    ...
    repositoryParts.some((part) => !/^[A-Za-z0-9._-]+$/u.test(part))
  ) {
    throw new Error(
      "mtm-coding: skill repository must be a GitHub HTTPS repository",
    );
  }
  if (
    ...
    pathParts.some(
      (part) =>
        part === "" || part === "." || part === ".." || !PATH_PART.test(part),
    )
  ) {
...
function validateSkillFiles(
  source: MtmCodingSkillSource,
): ValidatedSkillFile[] {
  if (source.files.length === 0)
    throw new Error("mtm-coding: skill package has no files");
  ...
  for (const file of source.files) {
    ...
    if (document && documents.has(file.name))
      throw new Error(
        "mtm-coding: duplicate skill document metadata for " + file.name,
      );
    names.add(file.name);
...
async function existingFile(path: string): Promise<boolean> {
  try {
    ...
    if (entry.isSymbolicLink() || !entry.isFile())
      throw new Error(
        "mtm-coding: skill resource is not a regular file: " + path,
      );
    return true;
  } catch (error) {
    if (errno(error) === "ENOENT") return false;
...
async function ensureDirectory(path: string): Promise<void> {
  try {
    ...
    if (entry.isSymbolicLink() || !entry.isDirectory())
      throw new Error(
        "mtm-coding: skill resource parent is not a regular directory: " + path,
      );
  } catch (error) {
...
function validateSkillDocument(file: ValidatedSkillFile, body: Buffer): void {
  ...
  if (documentName !== file.file.name)
    throw new Error(
      "mtm-coding: skill document name mismatch for " + file.file.name,
    );
}

function assertNotAborted(signal: AbortSignal | undefined): void {
  if (signal?.aborted)
    throw new Error("mtm-coding: skill installation cancelled");
}
...
async function responseBytes(
  ...
  if (!response.ok)
    throw new Error(
      "mtm-coding: skill download failed with HTTP " +
        response.status +
        ": " +
        url,
    );
  ...
  if (response.body === null)
    throw new Error("mtm-coding: skill download returned an empty body");
...
function parseSkillDocumentName(document: string): string | undefined {
  ...
  if (
    firstLineEnd < 0 ||
    document.slice(0, firstLineEnd).replace(/\r$/u, "") !== "---"
  )
    return undefined;
  ...
  while (lineStart <= document.length) {
    ...
    if (line === "---") {
      const parsed = parseYaml(
        document.slice(firstLineEnd + 1, lineStart),
      ) as unknown;
      if (
        parsed === null ||
...
async function completeExistingPackage(
  root: string,
  files: readonly ValidatedSkillFile[],
  source: MtmCodingSkillSource,
  fetchImpl: FetchLike,
  signal?: AbortSignal,
): Promise<void> {
  for (const file of files) {
    ...
    try {
      ...
      if (errno(error) !== "EEXIST" || !(await existingFile(target)))
        throw error;
    }
  }
}
...
async function install(
  ...
  if (!SKILL_NAME.test(id))
    throw new Error("mtm-coding: invalid package id " + id);
  ...
  try {
    ...
    await writeFile(
      join(temporary, ".mtmharness.json"),
      JSON.stringify({ packageId: id, status: "installed", source }, null, 2) +
        "\n",
      { flag: "wx" },
    );
    try {
...
export function skillPackageRoot(
  id: string,
...
export async function ensureSkillPackage(
  ...
  if (pending === undefined) {
    ...
    pending
      .finally(() => {
        if (installing.get(root) === pending) installing.delete(root);
      })
      .catch(() => {});
  }
  return pending;
...
export function applyFileSkills(
  ctx: Context,
  providerName: string,
  root: string,
): void {
