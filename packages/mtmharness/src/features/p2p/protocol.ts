...
export type P2pStatus =
  ...
  | "closed"
  | "error";
export type P2pDiscoveryStatus =
  | "idle"
  | "discovering"
  | "connected"
  | "failed";
...
export type P2pMessage =
  ...
  | { type: "socket"; request: P2pHttpRequest; port: MessagePort }
...
export function validateRequest(
  ...
  if (request.peerId !== undefined && request.peerId.length === 0)
    throw new Error("peer id is required");
  if (request.address !== undefined && request.address.length === 0)
    throw new Error("peer address is required");
  if (
    !(request.body instanceof Uint8Array) ||
    request.body.byteLength > maxBodyBytes
  ) {
    throw new Error("request body exceeds maximum");
  }
  if (!/^[A-Z0-9!#$%&'*+.^_`|~-]+$/.test(request.method))
    throw new Error("invalid HTTP method");
  if (!request.target.startsWith("/") || request.target.includes("#"))
