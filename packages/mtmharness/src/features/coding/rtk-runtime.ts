...
import { type CollectedRun, runCollected } from "./runtime.js";
...
const RELEASE_BASE =
  "https://github.com/rtk-ai/rtk/releases/download/v" + RTK_VERSION;
...
const ASSETS: readonly RtkAsset[] = [
  {
    ...
    sha256: "c4c036fb181fc55ef329786c8c17e0d427972b053b825944d968a6aafef1ba4",
    archive: "tar.gz",
    binary: "rtk",
  },
  {
...
export function rtkDisabled(command: string): boolean {
  return /^(?:[A-Za-z_][A-Za-z0-9_]*=[^\s]*\s+)*RTK_DISABLED=1(?:\s|$)/.test(
    command.trim(),
  );
}
...
async function canReclaimInstallLock(lockPath: string): Promise<boolean> {
  try {
    if (Date.now() - (await stat(lockPath)).mtimeMs <= INSTALL_LOCK_STALE_MS)
      return false;
    let owner: string;
    try {
...
export function rtkEnvironment(
  home: string,
...
async function prepareRtkEnvironment(
  home: string,
  env: Readonly<Record<string, string>>,
  ...
  await writeFile(
    ...
    [
      "[tracking]",
      "enabled = false",
      "database_path = " + JSON.stringify(join(home, "history.db")),
      "[tee]",
      "enabled = false",
      'mode = "never"',
      "[telemetry]",
...
function targetFromTar(buffer: Buffer, asset: RtkAsset): Buffer {
  ...
  while (offset + 512 <= data.length) {
    ...
    if (!validArchivePath(fullName))
      throw new Error("RTK archive contains an unsafe path");
    const size = parseOctal(header.subarray(124, 136));
    if (!Number.isSafeInteger(size) || size < 0 || size > MAX_ARCHIVE_BYTES)
      throw new Error("RTK archive entry exceeds the size limit");
    ...
    if (type === 0 || type === 48) {
      if (fullName === asset.binary) {
        if (found !== undefined)
          throw new Error(
            "RTK archive contains duplicate " + asset.binary + " entries",
          );
  ...
  if (found === undefined)
    throw new Error("RTK archive does not contain " + asset.binary);
  return found;
}
...
function targetFromZip(buffer: Buffer, asset: RtkAsset): Buffer {
  ...
  if (directoryOffset + directorySize > buffer.length)
    throw new Error("RTK zip directory is truncated");
  let offset = directoryOffset;
  let found: Buffer | undefined;
  let expanded = 0;
  for (let index = 0; index < count; index++) {
    if (buffer.readUInt32LE(offset) !== 0x02014b50)
      throw new Error("RTK zip entry is malformed");
    ...
    if (!validArchivePath(name))
      throw new Error("RTK zip contains an unsafe path");
    if (((externalAttributes >>> 16) & 0xf000) === 0xa000)
      throw new Error("RTK zip contains a symlink");
    ...
    if (
      ...
      throw new Error("RTK zip local entry is malformed");
    ...
    if (compressed.length !== compressedSize)
      throw new Error("RTK zip entry is truncated");
    if (size > MAX_ARCHIVE_BYTES - expanded)
      throw new Error("RTK zip expanded output exceeds the size limit");
    ...
    const body =
      method === 0
        ...
        : method === 8
          ...
          : undefined;
    if (body === undefined || body.length !== size)
      throw new Error("RTK zip compression is unsupported");
    expanded += body.length;
    if (name === asset.binary) {
      if (found !== undefined)
        throw new Error(
          "RTK zip contains duplicate " + asset.binary + " entries",
        );
      found = body;
    }
  }
  if (found === undefined)
    throw new Error("RTK zip does not contain " + asset.binary);
  return found;
}

export function extractRtkBinary(buffer: Buffer, asset: RtkAsset): Buffer {
  if (buffer.length > MAX_ARCHIVE_BYTES)
    throw new Error("RTK archive exceeds the size limit");
  return asset.archive === "zip"
    ? targetFromZip(buffer, asset)
    : targetFromTar(buffer, asset);
}
...
async function readResponseBytes(
  ...
  if (declared !== null && Number.parseInt(declared, 10) > MAX_ARCHIVE_BYTES)
    throw new Error("RTK download exceeds the size limit");
...
function pinnedChecksum(checksums: string, asset: RtkAsset): boolean {
  return checksums.split(/\r?\n/).some((line) => {
    ...
    return (
      match !== null &&
      match[1]!.toLowerCase() === asset.sha256 &&
      match[2]!.trim() === asset.name
    );
  });
}
...
async function probe(
  ...
  try {
    ...
    return (
      ...
      new RegExp("\\b" + RTK_VERSION.replaceAll(".", "\\.") + "\\b").test(
        result.stdout,
      )
    );
  } catch {
...
async function install(
  ...
  try {
    ...
    const checksums = (
      await download(RELEASE_BASE + "/checksums.txt", fetchImpl, signal)
    ).toString("utf8");
    if (!pinnedChecksum(checksums, asset) || digest(archive) !== asset.sha256)
    ...
    if (!(await probe(ctx, tempBinary, cwd, env, signal)))
      throw new Error("RTK version probe failed after install");
...
export async function ensureRtk(
...
): Promise<{ command: string; home: string; env: Record<string, string> }> {
  ...
  if (asset === undefined)
    throw new Error(
      "RTK is not available for " + process.platform + "/" + process.arch,
    );
  ...
  if (pending === undefined) {
    ...
    pending
      .finally(() => {
        if (installing.get(key) === pending) installing.delete(key);
      })
      .catch(() => {});
  }
...
export function shouldRewriteRtk(command: string): boolean {
  const trimmed = command.trim();
  return (
    trimmed.length > 0 && !/^rtk(?:\s|$)/.test(trimmed) && !rtkDisabled(command)
  );
}
...
export async function rewriteRtk(
  ...
  if (
    result.timedOut ||
    (result.outcome.exitCode !== 0 && result.outcome.exitCode !== 3)
  )
    return undefined;
  ...
  if (
    ...
    return undefined;
  return {
    command: bindRtkExecutable(rewritten, binary),
