...
export type MtmCodingCardProps = PropsRuntime<"settings.plugin.item"> &
    ...
    <div style={fieldStyle}>
      ...
      <label style={{ alignItems: "center", display: "flex", gap: 8 }}>
        <input
          type="checkbox"
          role="switch"
          aria-checked={props.field.text === "true"}
...
export function MtmCodingCard(props: MtmCodingCardProps) {
  const t = props.t;
  const state = props.useMtmCodingCard(
    (snapshot: MtmCodingCardState) => snapshot,
  );
    ...
    <li style={cardStyle}>
      ...
      >
        <span>
          ...
          <span style={{ display: "block", fontSize: 12, opacity: 0.68 }}>
            {t("description")}
          </span>
        </span>
      ...
      {open ? (
        <div style={bodyStyle}>
          ...
          <BooleanField
            t={t}
            label="codebaseMemoryEnabled"
            hint="codebaseMemoryEnabledHint"
            field={state.fields.codebaseMemoryEnabled}
            disabled={disabled}
            onChange={(value) => {
              props.edit("codebaseMemoryEnabled", String(value));
            }}
            onReset={() => {
              props.resetField("codebaseMemoryEnabled");
            }}
          />
          <BooleanField
            t={t}
          ...
          <BooleanField
            ...
            disabled={disabled}
            onChange={(value) => {
              props.edit("codebaseMemoryAugmentHooks", String(value));
            }}
            onReset={() => {
              props.resetField("codebaseMemoryAugmentHooks");
            }}
          />
          <BooleanField
            t={t}
            label="ponytailEnabled"
            hint="ponytailEnabledHint"
            field={state.fields.ponytailEnabled}
            disabled={disabled}
            onChange={(value) => {
              props.edit("ponytailEnabled", String(value));
            }}
            onReset={() => {
              props.resetField("ponytailEnabled");
            }}
          />
          <div style={fieldStyle}>
            {fieldLabel(
              ...
              mode,
              () => {
                props.resetField("ponytailMode");
              },
              disabled,
            )}
            <select
              aria-label={t("ponytailMode")}
              value={mode.text}
              disabled={disabled}
              onChange={(event) => {
                props.edit("ponytailMode", event.target.value);
              }}
            >
              {MODE_VALUES.map((value) => (
                <option key={value} value={value}>
                  {t(
                    ("mode" +
                      value[0].toUpperCase() +
                      value.slice(1)) as MtmCodingLocaleKey,
                  )}
                </option>
              ))}
            </select>
          ...
          <BooleanField
            t={t}
            label="ponytailSubagents"
            hint="ponytailSubagentsHint"
            field={state.fields.ponytailSubagents}
            disabled={disabled}
            onChange={(value) => {
              props.edit("ponytailSubagents", String(value));
            }}
            onReset={() => {
              props.resetField("ponytailSubagents");
            }}
          />
          <div style={fieldStyle}>
            {fieldLabel(
              ...
              rtkMode,
              () => {
                props.resetField("rtkMode");
              },
              disabled,
            )}
            <select
              aria-label={t("rtkMode")}
              value={rtkMode.text}
              disabled={disabled}
              onChange={(event) => {
                props.edit("rtkMode", event.target.value);
              }}
            >
              {RTK_MODE_VALUES.map((value) => (
                <option key={value} value={value}>
                  {t(
                    ("rtkMode" +
                      value[0].toUpperCase() +
          ...
          {update.available ? (
            <div style={fieldStyle}>
              ...
              {update.status === "available" ? (
                <p role="status">{t("updateAvailable")}</p>
              ) : null}
              {update.status === "current" ? (
                <p role="status">{t("upToDate")}</p>
              ) : null}
              {update.status === "updated" ? (
                <p role="status">{t("updateComplete")}</p>
              ) : null}
              {update.restartRequired ? (
                <p role="status">{t("restartRequired")}</p>
              ) : null}
              {update.error !== null ? (
              ...
              <div style={actionStyle}>
                ...
                >
                  {t(
                    update.checking ? "checkingForUpdates" : "checkForUpdates",
                  )}
                </button>
                <button
                  type="button"
                  ...
                  onClick={props.updatePackage}
                >
                  {t(update.updating ? "updatingPackage" : "updateNow")}
                </button>
              </div>
            </div>
          ) : null}
          <div style={actionStyle}>
            ...
            {state.dirty ? (
              <span style={{ marginRight: "auto", opacity: 0.68 }}>
                {t("unsaved")}
              </span>
            ) : null}
            <button
            ...
            <button
              ...
              style={buttonStyle}
              disabled={
                !state.dirty || state.invalid || state.saving || disabled
              }
              onClick={props.save}
            >
