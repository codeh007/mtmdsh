...
function internalTarget(pathname: string, search: string): string {
  try {
    return validateReturnTarget(pathname + search);
  } catch {
    return "/";
  }
}
...
function LoginView({
  ...
  const target = resolveLoginReturnTarget(location.searchStr);
  return (
    <main className="grid min-h-full place-items-center p-6">
      <div className="w-full max-w-sm space-y-4 rounded-lg border bg-card p-6 shadow-sm">
        <h1 className="font-semibold text-lg">Sign in to MTM Harness</h1>
        ...
        <button
          ...
          type="button"
          disabled={!auth.interactiveLogin}
          onClick={() =>
            void auth
              .beginLogin({ returnTarget: target })
        ...
        >
          {auth.interactiveLogin ? "Sign in" : "Authentication unavailable"}
        </button>
      </div>
    </main>
  );
...
function UnavailableView({
  ...
  return (
    <main className="grid min-h-full place-items-center p-6">
      <div className="w-full max-w-sm space-y-3 rounded-lg border border-destructive/40 bg-card p-6">
        <h1 className="font-semibold text-lg">Authentication unavailable</h1>
...
export function createClientRouter(options: ClientRouterOptions) {
  ...
  const unavailableRoute = createRoute({
    getParentRoute: () => rootRoute,
    path: "/unavailable",
    component: () => <UnavailableView auth={auth} />,
  });
  const authenticatedRoute = createRoute({
    ...
    beforeLoad: async ({ location }) => {
      await auth.ready;
      const status = auth.getSnapshot().status;
      ...
      throw redirect({
        to: "/login",
        search: {
          returnTo: internalTarget(location.pathname, location.searchStr),
        },
      });
    },
    component: () => <Outlet />,
