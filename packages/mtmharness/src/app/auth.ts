...
export type MtmHarnessAuthStatus =
  | "signed-out"
  | "discovering"
  | "ready"
  | "authorizing"
...
function canonicalIssuer(value: string): string {
  ...
  try {
    ...
    throw new OAuthError("OAuth issuer is invalid", "oauth_issuer_invalid");
  }
  if (
    url.protocol !== "https:" ||
    url.username ||
    ...
    throw new OAuthError(
      "OAuth issuer must be a canonical HTTPS URL",
...
function jsonPart(value: string, label: string): JsonRecord {
  try {
    return responseBody(
      JSON.parse(new TextDecoder().decode(decodeBase64Url(value, label))),
      label,
    );
  } catch (error) {
...
function callbackMatchesRedirect(
  callback: URL,
...
export class OAuthClient implements MtmHarnessAuthClient {
  ...
  private callbackReturnTarget: string | undefined;
  ...
  constructor(config: OAuthClientConfig, options: OAuthClientOptions = {}) {
    ...
    this.config = {
      ...
      ...(config.discoveryUrl === undefined
        ? {}
        : {
            discoveryUrl: urlValue(config.discoveryUrl, "discovery URL", true),
          }),
  ...
  getAccountPartition(): string | undefined {
    return this.tokens?.accountPartition;
  }

  getReturnTarget(): string | undefined {
    return this.callbackReturnTarget;
  }
  ...
  async discover(): Promise<OAuthDiscovery> {
    ...
    try {
      ...
      throw this.fail(
        new OAuthError(
          "OAuth discovery is unavailable",
    ...
    try {
      ...
      throw this.fail(
        new OAuthError(
          "OAuth discovery returned invalid data",
    ...
    try {
      ...
      if (issuer !== this.config.issuer)
        throw new OAuthError("OAuth issuer mismatch", "oauth_issuer_mismatch");
      const authorizationEndpoint = urlValue(
        item.authorization_endpoint,
        "authorization endpoint",
      );
      const tokenEndpoint = urlValue(item.token_endpoint, "token endpoint");
      const userinfoEndpoint =
        item.userinfo_endpoint === undefined
          ? undefined
          : urlValue(item.userinfo_endpoint, "userinfo endpoint");
      const jwksUri = urlValue(item.jwks_uri, "JWKS URI");
      const revocationEndpoint =
        item.revocation_endpoint === undefined
          ? undefined
          : urlValue(item.revocation_endpoint, "revocation endpoint");
      const endSessionEndpoint =
        item.end_session_endpoint === undefined
          ? undefined
          : urlValue(item.end_session_endpoint, "end-session endpoint");
      const responseTypes = responseArray(
        item.response_types_supported,
        "response types",
      );
      const grantTypes = responseArray(
        item.grant_types_supported,
        "grant types",
      );
      const challenges = responseArray(
        item.code_challenge_methods_supported,
        "PKCE methods",
      );
      const scopes = responseArray(item.scopes_supported, "scopes");
      const idTokenSigningAlgorithms = responseArray(
        item.id_token_signing_alg_values_supported,
        "ID token signing algorithms",
      );
      if (
        !idTokenSigningAlgorithms.some((algorithm) =>
          (SUPPORTED_ID_TOKEN_ALGORITHMS as readonly string[]).includes(
            algorithm,
          ),
        )
      )
        throw new OAuthError(
          "OAuth authority does not support a verified ID token",
          "oauth_id_token_algorithm_missing",
        );
      if (
        ...
        throw new OAuthError(
          "OAuth authority does not support the required flow",
          "oauth_capability_missing",
        );
      }
  ...
  async consumeCallback(callbackUrl?: string): Promise<boolean> {
    ...
    try {
      ...
      this.callbackReturnTarget = transaction.returnTarget;
      this.removeTransaction();
      if (
        !callbackMatchesRedirect(callback, transaction.redirectUri) ||
        transaction.redirectUri !== this.config.redirectUri ||
        transaction.issuer !== this.config.issuer ||
        transaction.clientId !== this.config.clientId ||
        transaction.resource !== this.config.resource
      ) {
        throw new OAuthError(
      ...
      if (state === null || state !== transaction.state)
        throw new OAuthError("OAuth state mismatch", "oauth_state_mismatch");
      const issuer = params.get("iss");
      if (issuer !== null && issuer !== this.config.issuer)
        throw new OAuthError("OAuth issuer mismatch", "oauth_issuer_mismatch");
      ...
      const subject = await this.verifyIdToken(
        tokenSet.idToken,
        transaction.nonce,
      );
  ...
  async getAccessToken(): Promise<string> {
    ...
    if (tokens === undefined)
      throw new OAuthError("Authentication is required", "auth_required", 401);
    if (tokens.expiresAt > this.now() + REFRESH_SKEW_MS)
  ...
  async logout(): Promise<void> {
    ...
    this.clear();
    if (tokens === undefined || metadata?.revocationEndpoint === undefined)
      return;
  ...
  private async exchangeCode(
    code: string,
    verifier: string,
  ): Promise<Omit<TokenSet, "accountPartition">> {
    const metadata = await this.discover();
  ...
  private async exchange(
    ...
    if (
      ...
      expiresIn <= 0 ||
      expiresIn * 1000 > MAX_ACCESS_TOKEN_LIFETIME_MS
    ) {
      throw new OAuthError(
    ...
    const scope =
      item.scope === undefined
        ? scopesFor(this.config).join(" ")
        : safeString(item.scope, "scope", 2048);
    ...
    if (item.resource !== undefined && item.resource !== this.config.resource)
      throw new OAuthError(
    ...
    const refreshToken =
      item.refresh_token === undefined
        ? undefined
        : safeString(item.refresh_token, "refresh token");
    const idToken =
      item.id_token === undefined
        ? undefined
        : safeString(item.id_token, "ID token", 16_384);
    if (requireIdToken && idToken === undefined)
      throw new OAuthError(
        "OAuth token response is missing an ID token",
  ...
  private async verifyIdToken(
    ...
    if (
      !metadata.idTokenSigningAlgorithms.includes(alg) ||
      !(SUPPORTED_ID_TOKEN_ALGORITHMS as readonly string[]).includes(alg)
    )
      throw new OAuthError(
        "OAuth ID token algorithm is not trusted",
        "oauth_id_token_algorithm_invalid",
      );
    if (header.typ !== undefined && header.typ !== "JWT")
      throw new OAuthError(
        "OAuth ID token type is invalid",
        "oauth_id_token_invalid",
      );
    const iss = safeString(payload.iss, "ID token issuer", 512);
    if (iss !== metadata.issuer)
      throw new OAuthError(
        "OAuth ID token issuer mismatch",
        "oauth_id_token_issuer_mismatch",
      );
    const audiences =
      typeof payload.aud === "string"
        ? [payload.aud]
        : responseArray(payload.aud, "ID token audience");
    if (!audiences.includes(this.config.clientId))
      throw new OAuthError(
        "OAuth ID token audience mismatch",
        "oauth_id_token_audience_mismatch",
      );
    if (audiences.length > 1 && payload.azp !== this.config.clientId)
      throw new OAuthError(
        "OAuth ID token authorized party mismatch",
        "oauth_id_token_audience_mismatch",
      );
    ...
    if (
      ...
      throw new OAuthError(
        "OAuth ID token has expired",
        "oauth_id_token_expired",
      );
    ...
    if (jwk === undefined)
      throw new OAuthError(
        "OAuth ID token signing key is unavailable",
        "oauth_id_token_key_missing",
      );
    if (jwk.use !== undefined && jwk.use !== "sig")
      throw new OAuthError(
        "OAuth ID token signing key is invalid",
        "oauth_id_token_key_invalid",
      );
    if (jwk.alg !== undefined && jwk.alg !== alg)
      throw new OAuthError(
        "OAuth ID token signing key algorithm mismatch",
        "oauth_id_token_key_invalid",
      );
    ...
    try {
      ...
      throw new OAuthError(
        "OAuth ID token signature is invalid",
  ...
  private async fetchJwks(endpoint: string): Promise<JsonRecord[]> {
    ...
    const keys = responseArrayOfRecords(
      responseBody(value, "JWKS").keys,
      "JWKS keys",
    );
    return keys;
  }
  ...
  private readTransaction(): OAuthTransaction {
    ...
    try {
      value = this.storage.getItem(storageKey(this.config));
    } catch {
      throw new OAuthError(
        "OAuth login state is unavailable",
        "oauth_storage_unavailable",
  ...
  private removeTransaction(): void {
    try {
      this.storage?.removeItem(storageKey(this.config));
    } catch {
  ...
  private ensureActive(): void {
    if (this.disposed)
      throw new OAuthError("OAuth client has been disposed", "oauth_disposed");
  }
}
...
export class MemoryTokenSource implements MtmHarnessTokenSource {
  ...
  getAccessToken(): Promise<string> {
    return this.token === undefined
      ? Promise.reject(
          new OAuthError("Authentication is required", "auth_required", 401),
        )
      : Promise.resolve(this.token);
  }

  getAccountPartition(): string | undefined {
    return this.token === undefined ? undefined : this.partition;
  }
...
export function createMemoryTokenSource(
  accessToken: string,
  accountPartition?: string,
): MemoryTokenSource {
...
export interface MtmHarnessAuthCoordinator extends MtmHarnessAuthClient {
  readonly interactiveLogin: boolean;
  readonly ready: Promise<void>;
}

class AuthCoordinator implements MtmHarnessAuthCoordinator {
  readonly interactiveLogin: boolean;
  readonly ready: Promise<void>;
  private snapshot: MtmHarnessAuthSnapshot;
  private readonly listeners = new Set<Listener>();
