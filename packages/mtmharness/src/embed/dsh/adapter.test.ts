...
describe("DshApiClient", () => {
  it("sends the official request envelope and derives session titles from projections", async () => {
    const fetchMock = vi.fn(async (input: string | URL, init?: RequestInit) => {
      const request = JSON.parse(String(init?.body)) as { type: string; rpcId: string; method: string; payload: unknown };
      ...
      return Response.json({ type: "server-response", rpcId: request.rpcId, result: { ok: true, value: { items: [{ sessionId: "session-1", updatedAt: 100, running: false, blank: false, cwd: "/workspace/project", projections: { asOfSeq: 4, values: { title: "Plan" } } }] } } });
    });
  ...
  it("accepts the official workspace and history result shapes", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (_input, init) => {
      ...
      return Response.json({ type: "server-response", rpcId: request.rpcId, result: { ok: true, value: values.shift() } });
    }));
  ...
  it("propagates the selected sandbox scope on every DSH request", async () => {
    const fetchMock = vi.fn(async (input: string | URL, init?: RequestInit) => {
      ...
      return Response.json({ type: "server-response", rpcId: (JSON.parse(String(init?.body)) as { rpcId: string }).rpcId, result: { ok: true, value: { items: [] } } });
    });
  ...
  it("preserves top-level and operation error codes", async () => {
    const fetchMock = vi.fn()
      ...
      .mockImplementationOnce(async (_input, init) => Response.json({ type: "server-response", rpcId: (JSON.parse(String(init?.body)) as { rpcId: string }).rpcId, result: { ok: false, error: { code: "title-invalid", message: "Title is invalid", details: {} } } }));
    vi.stubGlobal("fetch", fetchMock);
    const client = new DshApiClient("https://api.example.test", "test-token");
  ...
  it("rejects malformed response envelopes and rpcId mismatches", async () => {
    const fetchMock = vi.fn(async (_input: string | URL, init?: RequestInit) => {
      const request = JSON.parse(String(init?.body)) as { rpcId: string };
      return Response.json({ type: "server-response", rpcId: request.rpcId + "-wrong", result: { ok: true, value: { items: [] } } });
    });
    const client = new DshApiClient("https://api.example.test", { tokenProvider: () => "test-token", fetch: fetchMock });

    await expect(client.listSessions()).rejects.toMatchObject({ message: "The server returned a mismatched response.rpcId" });

    fetchMock.mockImplementationOnce(async () => Response.json({ rpcId: "missing-type", result: { ok: true, value: { items: [] } } }));
    await expect(client.listSessions()).rejects.toMatchObject({ message: "The server returned an invalid response.type" });
  });

  it("requires structured operation errors", async () => {
    const fetchMock = vi.fn(async (_input: string | URL, init?: RequestInit) => {
      const request = JSON.parse(String(init?.body)) as { rpcId: string };
      return Response.json({ type: "server-response", rpcId: request.rpcId, result: { ok: false, error: { code: "unavailable", message: "Unavailable" } } });
    });

    await expect(new DshApiClient("https://api.example.test", { tokenProvider: () => "test-token", fetch: fetchMock }).listSessions()).rejects.toMatchObject({ message: "The server returned an invalid operation error.details" });
  });

  it("rejects malformed operation values before they reach the UI", async () => {
    vi.stubGlobal("fetch", vi.fn(async (_input, init) => {
      const request = JSON.parse(String(init?.body)) as { rpcId: string };
      return Response.json({ type: "server-response", rpcId: request.rpcId, result: { ok: true, value: { items: [{ sessionId: "session-1" }] } } });
    }));
  ...
  it("reports unavailable WebSocket transport without using a legacy endpoint", async () => {
    ...
    await expect(client.openSocket({ sandboxId: "sbx_1", channel: "mux" })).rejects.toMatchObject({ code: "websocket_unavailable" });
  });
});
