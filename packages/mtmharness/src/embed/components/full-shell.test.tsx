...
function createRuntimeOptions(client: DshClient, sandboxClient: SandboxClient) {
  return {
    ...
    webSocketFactory: (url: URL, protocols: readonly string[]) =>
      new TestWebSocket(url, protocols) as unknown as WebSocket,
    client,
    sandboxClient,
  };
}
...
const secondSandbox: SandboxRecord = {
  ...sandbox,
  id: "sbx_00000000-0000-4000-8000-000000000002",
  workspaceId: "ws_00000000-0000-4000-8000-000000000002",
  name: "Project",
};
  ...
  return {
    ...
    openSocket: async (_input, factory) => {
      if (factory === undefined)
        throw new Error("fixture socket factory is required");
...
function createLiveClient(): DshClient {
  const workspace: DshWorkspaceView = {
  ...
  const session: MtmSessionSummary = {
...
describe("FullShellFrame", () => {
  it("renders the official shell slot composition and disposes cleanly", async () => {
    ...
    const runtime = new MtmHarnessRuntime(
      "https://api.example.test",
      createRuntimeOptions(createFixtureClient(), createSandboxClient()),
    );
    ...
    expect(
      container.querySelector('[data-shell-profile="full"]'),
    ).not.toBeNull();
    expect(container.querySelector('[data-runtime="adapter"]')).not.toBeNull();
    for (const slot of FULL_SHELL_SLOTS)
      expect(
  ...
  it("renders and switches the sandbox selector without losing the shell", async () => {
    ...
    const runtime = new MtmHarnessRuntime(
      "https://api.example.test",
      createRuntimeOptions(createLiveClient(), createSandboxClient()),
    );
    ...
    const selector = container.querySelector(
      'select[aria-label="Sandbox"]',
    ) as HTMLSelectElement;
    expect(selector).not.toBeNull();
    expect(selector.value).toBe(sandbox.id);
    expect(
      container.querySelector('[data-slot="sandbox.control"]')?.textContent,
    ).toContain("ready");
  ...
  it("renders live registry sessions and selects one from the sidebar", async () => {
    ...
    expect(
      container.querySelector('[data-slot="conversation.session.header"]')
        ?.textContent,
