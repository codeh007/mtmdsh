  ...
  return (
    <section
      className={cn(
        ...
        compact ? "bg-background" : "rounded-xl border bg-card shadow-sm",
      )}
      aria-label="DSH conversation"
    >
      {showHeader ? (
        <>
          <div
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-muted-foreground text-xs">
                  One DSH session, streamed from the host
                </p>
              </div>
            </div>
      ...
      {snapshot.status === "auth-required" ? (
        <div
          className="mx-4 mt-4 rounded-lg border border-border bg-muted/40 px-3 py-2 text-sm"
          role="status"
        >
          Sign in above or continue anonymously on the sign-in page to start a
          conversation.
        </div>
      ) : null}
      {snapshot.status === "error" ? (
        <div
          className="mx-4 mt-4 flex items-center justify-between gap-3 rounded-lg border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm"
          role="alert"
        >
          <span>
            {snapshot.error ?? "The conversation is temporarily unavailable."}
          </span>
          <Button
            type="button"
            size="sm"
        ...
        {empty ? (
          <div className="m-auto max-w-xs text-center">
            ...
            <p className="mt-1 text-muted-foreground text-sm leading-6">
              Your session history and streamed replies will appear here.
            </p>
          </div>
        ) : null}
      ...
      <Separator />
      <form
        className={cn("flex items-end gap-2", compact ? "p-3" : "p-4")}
        onSubmit={submit}
      >
        <label className="sr-only" htmlFor="mtmharness-prompt">
          Message
        </label>
        <Textarea
        ...
        <Button
          ...
          disabled={
            !draft.trim() || busy || snapshot.status === "auth-required"
          }
        >
