...
function statusLabel(snapshot: RuntimeSnapshot): string {
  if (
    snapshot.sandboxCatalogStatus === "loading" ||
    snapshot.operation === "switching-sandbox"
  )
  ...
  if (
    snapshot.registryStatus === "loading" ||
    snapshot.operation === "refreshing"
  )
    return "Loading";
  ...
  return snapshot.selectedSessionId === undefined
    ? "Not connected"
    : "Disconnected";
}
...
function statusVariant(
  ...
  if (
    ...
    snapshot.registryStatus === "error" ||
    snapshot.sandboxCatalogStatus === "error" ||
...
function SandboxControl({
  ...
  const selected = snapshot.sandboxes.find(
    (sandbox) => sandbox.id === snapshot.selectedSandboxId,
  );
  const busy =
    snapshot.sandboxCatalogStatus === "loading" ||
    snapshot.operation === "switching-sandbox" ||
    snapshot.operation === "creating-sandbox";
  const status = busy
    ? snapshot.operation === "creating-sandbox"
      ? "Creating"
      : "Switching"
  ...
  return (
    <div
      data-slot="sandbox.control"
      className="flex min-w-0 flex-1 max-w-[18rem] flex-col gap-1 sm:flex-none"
    >
      <label
        className="text-muted-foreground text-[0.68rem]"
        htmlFor="full-shell-sandbox"
      >
        Sandbox
      </label>
      <div className="flex min-w-0 items-center gap-1">
        <select
          ...
          onChange={(event) =>
            runRuntimeAction(runtime.selectSandbox(event.target.value))
          }
        >
          {snapshot.sandboxes.length === 0 ? (
            <option value="">No sandbox selected</option>
          ) : null}
          {snapshot.sandboxes.map((sandbox) => (
            <option
              key={sandbox.id}
              value={sandbox.id}
              disabled={sandbox.status === "destroyed"}
            >
...
function SessionRow({
  ...
  return (
    <button
      ...
      className={cn(
        "flex min-h-8 w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm",
        selected
...
function WorkspaceSessionTree({
  ...
  if (
    ...
    return (
      <div
        ...
        className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto px-3 py-3 text-muted-foreground text-xs"
      >
        <span>Loading workspaces</span>
        <span className="h-2 w-4/5 animate-pulse rounded bg-muted" />
        <span className="h-2 w-3/5 animate-pulse rounded bg-muted" />
      </div>
    );
  }
      ...
      >
        <div className="flex items-center gap-2 text-destructive text-xs">
          ...
          <span className="min-w-0">
            {snapshot.registryError ?? "Workspace data is unavailable."}
          </span>
        </div>
        <Button
          type="button"
          ...
          variant="outline"
          onClick={onRetry}
          disabled={snapshot.operation !== undefined}
        >
          Retry
        </Button>
      </div>
  ...
  const unassigned = snapshot.sessions.filter(
    (session) =>
      !archived.has(session.sessionId) && !assigned.has(session.sessionId),
  );
  ...
  if (workspaceItems.length === 0 && unassigned.length === 0) {
    return (
      <div
        data-slot="sidebar.workspaces"
        className="flex min-h-0 flex-1 flex-col items-center justify-center gap-2 px-3 py-6 text-center text-muted-foreground text-xs"
      >
  ...
  if (collapsed) {
    return (
      <section
        data-slot="sidebar.workspaces"
        className="flex min-h-0 flex-1 flex-col items-center gap-3 overflow-y-auto px-2 py-3"
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-2 pb-2 text-muted-foreground text-[0.68rem] uppercase">
      ...
      >
        {workspaceItems.map(({ workspace, sessions }) => (
          <div key={workspace.workspaceId}>
            ...
            {sessions.length > 0 ? (
              <fieldset className="ml-4 border-border border-l pl-2">
                {sessions.map((session) => (
                  <SessionRow
                    key={session.sessionId}
                    session={session}
                    selected={session.sessionId === snapshot.selectedSessionId}
                    onSelect={() => onSelect(session.sessionId)}
                  />
                ))}
              </fieldset>
            ) : (
              <p className="ml-8 py-1 text-muted-foreground text-xs">
                No sessions
              </p>
            )}
          </div>
...
function FullShellSidebar({
  ...
  const selectedWorkspace =
    snapshot.workspaces.find((workspace) =>
      workspace.sessionIds.includes(snapshot.selectedSessionId ?? ""),
    ) ?? snapshot.workspaces[0];
  const busy =
    snapshot.operation !== undefined || snapshot.registryStatus === "loading";
  return (
    <>
      {mobileOpen ? (
        <button
      ...
      <aside
        data-slot="sidebar"
        className={cn(
          "fixed inset-y-0 -left-72 z-40 flex w-72 flex-col border-border border-r bg-card transition-[width,left] md:relative md:left-0 md:z-0",
      ...
      >
        ...
        >
          ...
          {!collapsed ? (
            <div className="min-w-0 flex-1">
              <p className="truncate font-semibold text-sm">DSH Web Client</p>
          ...
          <Button
            ...
            onClick={onToggle}
          >
            {collapsed ? <ChevronRight /> : <PanelLeft />}
          </Button>
        </div>
        <Separator />
        <div
        ...
        >
          <Button
            ...
            className={cn("w-full", collapsed && "md:w-8")}
            onClick={() =>
              runRuntimeAction(
                runtime.createSession(selectedWorkspace?.workspaceId),
              )
            }
            disabled={
              busy ||
              snapshot.status === "auth-required" ||
              snapshot.selectedSandboxId === undefined
            }
        ...
        <WorkspaceSessionTree
          snapshot={snapshot}
          collapsed={collapsed}
          onRetry={() => runRuntimeAction(runtime.refreshRegistry())}
          onSelect={(sessionId) => {
            onSelect(sessionId);
            onCloseMobile();
          }}
        ...
        >
          <Button
            ...
            className={cn(
              ...
              collapsed && "md:w-8 md:justify-center",
            )}
            disabled
...
function SessionHeader({
  ...
  const selected = snapshot.sessions.find(
    (session) => session.sessionId === snapshot.selectedSessionId,
  );
  const workspace = snapshot.workspaces.find((item) =>
    item.sessionIds.includes(selected?.sessionId ?? ""),
  );
  const [renaming, setRenaming] = useState(false);
  const [draft, setDraft] = useState(() =>
    selected ? sessionTitle(selected) : "",
  );
  ...
  return (
    <header
      ...
      className="flex min-h-16 shrink-0 items-center justify-between gap-4 border-border border-b px-4 py-3 md:px-6"
    >
      <div className="min-w-0 flex-1">
        {renaming && selected ? (
          <form
            ...
            <div className="flex items-center gap-2">
              ...
              {selected ? (
                <span
                  className={cn(
                    "size-2 rounded-full",
                    selected.running ? "bg-amber-500" : "bg-emerald-500",
                  )}
                  title={
                    selected.running ? "Session is running" : "Session is ready"
                  }
                  role="img"
                  aria-label={
                    selected.running ? "Session is running" : "Session is ready"
      ...
      <div className="flex shrink-0 items-center gap-1">
        ...
        <Button
          ...
          onClick={() => {
            if (selected)
              runRuntimeAction(runtime.forkSession(selected.sessionId));
          }}
          disabled={!selected || busy}
          title="Fork session"
        ...
        <Button
          ...
          variant="ghost"
          size="icon-sm"
          onClick={() => runRuntimeAction(runtime.refreshRegistry())}
          disabled={busy}
...
function DetailsPanel({
  ...
  const selectedSandbox = snapshot.sandboxes.find(
    (sandbox) => sandbox.id === snapshot.selectedSandboxId,
  );
  const workspace = snapshot.workspaces.find((item) =>
    item.sessionIds.includes(selected?.sessionId ?? ""),
  ...
  return (
    <aside
      data-slot="details"
      className="hidden w-72 shrink-0 flex-col border-border border-l bg-card xl:flex"
    >
      <div className="flex h-16 items-center justify-between border-border border-b px-4">
        <div>
          ...
          <p className="mt-1 text-muted-foreground text-xs">Selected session</p>
        </div>
      ...
      {selected ? (
        <dl className="space-y-4 px-4 py-5 text-sm">
          <div>
            <dt className="text-muted-foreground text-xs">Title</dt>
            <dd className="mt-1 break-words">{sessionTitle(selected)}</dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd className="mt-1 break-words">
              {selectedSandbox?.name ?? "Unselected"}
            </dd>
          </div>
          <div>
            <dt className="text-muted-foreground text-xs">Sandbox ID</dt>
            <dd className="mt-1 break-all font-mono text-xs">
              {selectedSandbox?.id ?? "-"}
            </dd>
          </div>
          <div>
            <dt className="text-muted-foreground text-xs">Workspace ID</dt>
            <dd className="mt-1 break-all font-mono text-xs">
          ...
          <div>
            ...
            <dd className="mt-1 break-words">
              {workspace?.path ?? "Unassigned"}
            </dd>
          </div>
          <div>
            <dt className="text-muted-foreground text-xs">Updated</dt>
            <dd className="mt-1">{formatUpdatedAt(selected.updatedAt)}</dd>
          </div>
        </dl>
      ) : (
        <div className="flex flex-1 flex-col items-center justify-center gap-3 px-6 text-center text-muted-foreground">
          <CircleAlert aria-hidden="true" className="size-5" />
          <p className="text-sm">Create or select a session to see details.</p>
        </div>
      )}
    </aside>
  );
...
export function FullShellFrame({
  children,
  runtime,
  auth,
  onClose,
  ...
  return (
    <div
      data-slot="root"
      data-shell-profile="full"
      data-runtime="adapter"
      data-registry-status={snapshot.registryStatus}
      className="relative flex h-screen min-h-[560px] w-full overflow-hidden bg-background text-foreground"
    >
      <FullShellSidebar
        runtime={runtime}
        snapshot={snapshot}
        ...
        onSelect={(sessionId) =>
          runRuntimeAction(runtime.selectSession(sessionId))
        }
      />
      <main className="flex min-w-0 flex-1 flex-col">
        <header className="flex min-h-14 shrink-0 flex-wrap items-center justify-between gap-3 border-border border-b px-3 py-2 md:px-5">
          <div className="flex min-w-0 items-center gap-2">
            ...
            />
            <span className="truncate font-medium text-sm">DSH workspace</span>
          </div>
          <div className="flex w-full min-w-0 items-end gap-2 sm:w-auto sm:shrink-0">
            <SandboxControl runtime={runtime} snapshot={snapshot} />
            <AdapterBadge />
            ...
            {onClose ? (
              <Button
                ...
                size="icon-sm"
                onClick={onClose}
                title="Close full workspace"
                aria-label="Close full workspace"
              >
                <X />
              </Button>
            ) : null}
        ...
        <div className="relative flex min-h-0 flex-1">
          ...
          >
            <SessionHeader
              key={snapshot.selectedSessionId ?? "none"}
              runtime={runtime}
              snapshot={snapshot}
            />
            <div
            ...
            >
              <div data-slot="conversation.composer" className="h-full min-h-0">
                {children}
              </div>
            </div>
          </section>
        ...
        <div
          ...
          className={cn(
            "border-border border-t px-4 py-2 text-xs md:px-6",
            snapshot.error || snapshot.registryError || snapshot.sandboxError
...
export function WorkspaceOverview({
  ...
  return (
    <section
      className="flex h-full min-h-0 flex-col overflow-y-auto p-4 md:p-6"
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1 font-semibold text-xl">
            {snapshot.workspaces[0]?.title ?? "workspace"}
          </h2>
          ...
          <p className="mt-1 break-all font-mono text-muted-foreground text-xs">
            {snapshot.selectedSandboxId ?? "-"} / {snapshot.workspaceId ?? "-"}
          </p>
        </div>
        <AdapterBadge />
      </div>
      <div className="mt-6 grid gap-3 sm:grid-cols-3">
        {[
          [
            "Workspaces",
            String(snapshot.workspaces.length),
            snapshot.registryStatus === "loading"
              ? "Loading"
              : "Registry records",
          ],
          ...
          [
            "Archived",
            String(snapshot.archivedSessionIds.length),
            "Hidden from active tree",
          ],
        ].map(([label, value, hint]) => (
          <div key={label} className="border-border border bg-card p-4">
            <p className="text-muted-foreground text-xs">{label}</p>
            <p className="mt-2 truncate font-semibold text-sm">{value}</p>
            <p className="mt-1 truncate text-muted-foreground text-xs">
              {hint}
            </p>
          </div>
      ...
      <div className="mt-6 border-border border bg-card p-4">
        <div className="flex items-center gap-2">
          <Folder aria-hidden="true" className="size-4 text-primary" />
          <h3 className="font-medium text-sm">Session registry</h3>
        </div>
        <div className="mt-4 divide-y divide-border border-border border-y">
          {activeSessions.length === 0 ? (
            ...
            activeSessions.map((session) => (
              <button
                type="button"
                key={session.sessionId}
                className="flex w-full items-center gap-3 py-3 text-left text-sm hover:bg-accent"
                onClick={() =>
                  runRuntimeAction(runtime.selectSession(session.sessionId))
                }
              >
                <span
                  className={cn(
                    "size-2 shrink-0 rounded-full",
                    session.running ? "bg-amber-500" : "bg-muted-foreground/50",
                  )}
                />
