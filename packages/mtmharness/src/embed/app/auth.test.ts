...
async function signedIdToken(
  ...
  if (idTokenKeys === undefined) {
    idTokenKeys = (await crypto.subtle.generateKey({ name: "Ed25519" }, true, [
      "sign",
      "verify",
    ])) as CryptoKeyPair;
...
async function jwksResponse(): Promise<Response> {
  ...
  return Response.json({
    keys: [{ ...idTokenJwk, kid: "test-key", alg: "EdDSA", use: "sig" }],
  });
}
...
async function authorizationTokenResponse(
  nonce: string,
  accessToken = "access-1",
  refreshToken = "refresh-1",
  expiresIn = 300,
  ...
  return tokenResponse(
    ...
    refreshToken,
    expiresIn,
    await signedIdToken(signedNonce),
  );
}
...
async function authPartition(
  ...
  const digest = new Uint8Array(
    await crypto.subtle.digest(
      "SHA-256",
      new TextEncoder().encode(otherIssuer + "|" + subject),
    ),
  );
...
describe("OAuthClient", () => {
  it("generates S256 PKCE and stores only a partitioned transaction", async () => {
    ...
    expect(
      sessionStorage.getItem(oauthTransactionStorageKey(config)),
    ).not.toBeNull();
  ...
  it("accepts a path issuer with independent endpoints and caller scopes", async () => {
    ...
    expect(
      normalizeConfig({ apiOrigin: pathConfig.resource, oauth: pathConfig })
        .oauth,
    ).toMatchObject({
      issuer: pathIssuer,
    ...
    const stored = JSON.parse(
      sessionStorage.getItem(oauthTransactionStorageKey(pathConfig)) ?? "{}",
    ) as { state?: string };
    await expect(
      auth.consumeCallback(
        "http://localhost/callback?code=code&state=" +
          encodeURIComponent(stored.state ?? ""),
      ),
    ).rejects.toMatchObject({ code: "oauth_scope_missing" });

    const wrongIssuer = new OAuthClient(pathConfig, {
      fetch: async () =>
        Response.json({ ...pathMetadata, issuer: pathIssuer + "/other" }),
    });
    await expect(wrongIssuer.discover()).rejects.toMatchObject({
  ...
  it("consumes a callback once and sanitizes the URL", async () => {
    ...
    await expect(
      auth.consumeCallback(
        "http://localhost/callback?code=one-time-code&state=" +
    ...
    expect(auth.getSnapshot()).toMatchObject({
      ...
      accountPartition: expect.stringMatching(/^[0-9a-f]{64}$/u),
    });
    expect(auth.getSnapshot().accountPartition).not.toBe(
      await authPartition("other-issuer", "user-1"),
    );
    expect(
      sessionStorage.getItem(oauthTransactionStorageKey(config)),
    ).toBeNull();
  ...
  it("rejects mismatched state and untrusted discovery endpoints", async () => {
    ...
    await expect(
      auth.consumeCallback(
        "http://localhost/callback?code=code&state=wrong-state",
      ),
    ).rejects.toMatchObject({ code: "oauth_state_mismatch" });
    expect(
      sessionStorage.getItem(oauthTransactionStorageKey(config)),
    ).toBeNull();
    const evil = createAuth(
      vi.fn(async () =>
        discoveryResponse({ token_endpoint: "http://evil.example.test/token" }),
      ),
    );
    await expect(evil.discover()).rejects.toMatchObject({
      code: "oauth_endpoint_untrusted",
    });
    auth.dispose();
    evil.dispose();
  });
  ...
  it("rejects an ID token with a mismatched nonce", async () => {
    ...
    ).rejects.toMatchObject({ code: "oauth_id_token_nonce_mismatch" });
  ...
  it("does not let an old refresh overwrite a newly authenticated account", async () => {
    ...
    const fetcher = vi.fn(async (input: string | URL, init?: RequestInit) => {
      ...
      if (url.endsWith("/oauth2/token")) {
        ...
        return authorizationTokenResponse(
          currentNonce,
          body.get("code") === "second-code" ? "access-2" : "access-1",
        );
      }
      if (url.endsWith("/api/auth/jwks")) return jwksResponse();
      throw new Error("unexpected request");
    ...
    await auth.consumeCallback(
      "http://localhost/callback?code=first-code&state=" +
        encodeURIComponent(firstTransaction.state),
    );
    now += 280_000;
    ...
    await auth.consumeCallback(
      "http://localhost/callback?code=second-code&state=" +
        encodeURIComponent(secondTransaction.state),
    );
  ...
  it("refreshes once for concurrent callers and clears on invalid_grant", async () => {
    ...
    const { state, nonce } = transaction();
    await auth.consumeCallback(
      "http://localhost/callback?code=code&state=" + encodeURIComponent(state),
    );
    now += 280_000;
    await expect(
      Promise.all([auth.getAccessToken(), auth.getAccessToken()]),
  ...
  it("revokes both tokens on logout", async () => {
    ...
    await auth.logout();
    expect(auth.getSnapshot().status).toBe("signed-out");
    expect(calls.filter((url) => url.endsWith("/oauth2/revoke"))).toHaveLength(
      2,
    );
    await expect(auth.getAccessToken()).rejects.toMatchObject({
      code: "auth_required",
