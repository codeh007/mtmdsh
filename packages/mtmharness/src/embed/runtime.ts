...
export function applySessionEvent(
  ...
  if (event.type === "user/message") {
    const id =
      typeof data.id === "string"
        ? data.id
        : "user-" + (event.seq ?? createRpcId());
    const text = messageText(data);
    return [
      ...messages.filter(
        (item) => !(item.id.startsWith("local-") && item.text === text),
      ),
      { id, role: "user", text },
    ];
  }
  if (event.type === "assistant/chunk") {
    ...
    const text =
      chunk &&
      typeof chunk === "object" &&
      (chunk as JsonRecord).type === "text-delta" &&
      typeof (chunk as JsonRecord).text === "string"
        ? ((chunk as JsonRecord).text as string)
        : "";
  ...
  if (event.type === "assistant/message") {
    ...
    const id =
      ...
      typeof (message as JsonRecord).id === "string"
        ? ((message as JsonRecord).id as string)
        : "assistant-" + (event.seq ?? createRpcId());
    const liveId = "stream-" + (data.turn ?? 0) + "-" + (data.step ?? 0);
    return [
      ...messages.filter((item) => item.id !== liveId && item.id !== id),
      { id, role: "assistant", text: messageText(message) },
    ];
  }
  return messages;
}
...
export function foldHistory(entries: HistoryEntry[]): ChatMessage[] {
  return entries.reduce(
    (messages, entry) => applySessionEvent(messages, entry.event),
    [] as ChatMessage[],
  );
}
...
function normalizeError(error: unknown): MtmHarnessError {
  ...
  if (error instanceof DshApiError)
    return new MtmHarnessError(error.message, error.code);
...
function parseSocketFrame(
  raw: unknown,
  sessionId: string,
): ParsedSocketFrame | undefined {
...
export class MtmHarnessRuntime {
  ...
  constructor(apiOrigin: string, options: MtmHarnessRuntimeOptions = {}) {
    ...
    this.tokenSource =
      ...
      (options.accessToken === undefined
        ? undefined
        : new MemoryTokenSource(options.accessToken, options.accountPartition));
    this.accountPartitionHint = options.accountPartition;
    this.activeAccountPartition =
      this.tokenSource?.getAccountPartition() ?? this.accountPartitionHint;
    ...
    this.client =
      options.client ??
      new DshApiClient(apiOrigin, { tokenProvider, onAuthFailure });
    this.sandboxClient =
      options.sandboxClient ??
      new SandboxApiClient(apiOrigin, { tokenProvider, onAuthFailure });
    this.unsubscribeTokenSource =
      this.tokenSource?.subscribe((snapshot) => {
        this.handleAuthSnapshot(snapshot);
      }) ?? (() => {});
  }
  ...
  async connect(): Promise<void> {
    ...
    if (
      ...
      this.snapshot.selectedSessionId === this.sessionId &&
      this.snapshot.operation === undefined
    )
      return;
    this.connectPromise ??= this.enqueue(() => this.connectInternal()).finally(
      () => {
  ...
  async refreshRegistry(): Promise<void> {
    ...
    this.refreshPromise = this.enqueue(() =>
      this.refreshRegistryInternal(),
    ).finally(() => {
  ...
  async selectSession(sessionId: string): Promise<void> {
    ...
    await this.enqueue(async () => {
      if (
        ...
        this.socket?.readyState === WebSocket.OPEN &&
        this.snapshot.selectedSessionId === value
      ) {
  ...
  private async createSandboxInternal(name: string): Promise<string> {
    ...
    try {
      ...
      const selected = await this.sandboxClient.selectSandbox(
        created.sandbox.id,
      );
      this.assertSandboxGeneration(generation);
      this.applySandboxCatalog(
        {
          sandboxes: mergeSandboxes(this.snapshot.sandboxes, [
            created.sandbox,
            selected,
          ]),
          defaultSandbox: selected,
        },
        selected,
      );
      ...
      this.update({
        ...
        operation: undefined,
        sandboxError: normalized.message,
  ...
  private async selectSandboxInternal(sandboxId: string): Promise<void> {
    ...
    try {
      ...
      this.applySandboxCatalog(
        {
          sandboxes: mergeSandboxes(this.snapshot.sandboxes, [selected]),
          defaultSandbox: selected,
        },
        selected,
      );
      ...
      if (firstSessionId !== undefined)
        await this.selectSessionInternal(firstSessionId);
      this.update({
        status: "idle",
        operation: undefined,
        sandboxError: undefined,
  ...
  private beginSandboxSwitch(
    operation: "switching-sandbox" | "creating-sandbox",
  ): number {
  ...
  private assertSandboxGeneration(generation: number): void {
    if (generation !== this.sandboxGeneration || this.disposed)
      throw new MtmHarnessError(
        "Sandbox operation was superseded",
        "sandbox_operation_superseded",
      );
  }

  private async restoreSandboxSnapshot(
    previous: RuntimeSnapshot,
    generation: number,
  ): Promise<void> {
    if (generation !== this.sandboxGeneration || this.disposed) return;
    const selected = previous.sandboxes.find(
      (sandbox) => sandbox.id === previous.selectedSandboxId,
    );
    this.sessionId = undefined;
    this.client.setSandboxScope(
      selected === undefined
        ? undefined
        : { sandboxId: selected.id, workspaceId: selected.workspaceId },
    );
  ...
  private applySandboxCatalog(
    catalog: {
      sandboxes: SandboxRecord[];
      defaultSandbox: SandboxRecord | null;
    },
    selected: SandboxRecord,
  ): void {
    this.client.setSandboxScope({
    ...
    this.update({
      sandboxCatalogStatus: "idle",
      sandboxError: undefined,
      sandboxes: mergeSandboxes(
        catalog.sandboxes,
        catalog.defaultSandbox === null ? [] : [catalog.defaultSandbox],
      ),
  ...
  private async ensureSandboxInternal(): Promise<SandboxRecord> {
    const current = this.snapshot.sandboxes.find(
      (sandbox) => sandbox.id === this.snapshot.selectedSandboxId,
    );
    ...
    try {
      ...
      if (selected === null && catalog.sandboxes.length === 0) {
        const created =
          await this.sandboxClient.createSandbox("Default workspace");
        catalog = {
          sandboxes: mergeSandboxes(catalog.sandboxes, [created.sandbox]),
          defaultSandbox: created.defaultSandbox ?? created.sandbox,
        };
        selected = catalog.defaultSandbox;
      }
      if (selected === null)
        throw new MtmHarnessError(
          "No default sandbox is selected",
          "sandbox_default_missing",
        );
      ...
      const normalized = normalizeError(error);
      this.update({
        sandboxCatalogStatus: "error",
        sandboxError: normalized.message,
        status: isAuthError(normalized)
          ? "auth-required"
  ...
  private async renameSessionInternal(
    ...
    try {
      const result = await this.client.renameSession({
        sessionId,
        title: value,
      });
      this.update({
        ...
        sessions: this.snapshot.sessions.map((session) =>
          session.sessionId === sessionId
            ? {
                ...
                title: result.title,
                projections: {
                  ...(session.projections ?? { asOfSeq: -1, values: {} }),
                  values: {
                    ...(session.projections?.values ?? {}),
                    title: result.title,
                  },
                  asOfSeq: Math.max(
                    session.projections?.asOfSeq ?? -1,
                    result.seq,
                  ),
                },
              }
  ...
  private async forkSessionInternal(
    sessionId: string,
    atSeq?: number,
  ): Promise<string> {
    ...
    try {
      ...
      const workspace = this.snapshot.workspaces.find((item) =>
    ...
    try {
      ...
      this.update({
        ...
        messages: [
          ...
          { id: localId, role: "user", text: value },
        ],
        error: undefined,
      });
  ...
  private async connectInternal(): Promise<void> {
    ...
    try {
      ...
      if (!selectedId && storedSessionId) {
        try {
          const restored = await this.client.createSession({
      ...
      this.update({
        status: isAuthError(normalized) ? "auth-required" : "error",
        registryStatus:
          this.snapshot.registryStatus === "loading"
            ? "error"
            : this.snapshot.registryStatus,
        registryError: this.snapshot.registryError ?? normalized.message,
        operation: undefined,
        error: normalized.message,
      });
      throw normalized;
  ...
  private async refreshRegistryInternal(
    operation: RuntimeOperation = "refreshing",
  ): Promise<void> {
    this.update({
    ...
    try {
      ...
      this.update({
        ...
        registryError: normalized.message,
        operation: undefined,
        status: isAuthError(normalized)
          ? "auth-required"
          : this.snapshot.status,
  ...
  private async selectSessionInternal(sessionId: string): Promise<void> {
    ...
    try {
      ...
      if (previousSocket && previousSocket !== this.socket)
        this.closeSocketInstance(previousSocket);
  ...
  private async openSocket(
    ...
    if (scope === undefined)
      throw new MtmHarnessError(
        "Sandbox scope is required",
        "sandbox_scope_required",
      );
    if (this.sessionId === sessionId && this.selectionVersion === version)
      this.update({ connectionStatus: "connecting" });
    let socket: WebSocket;
    try {
      const request: DshSocketRequest = {
      ...
      const factory =
        this.webSocketFactory === undefined
          ? undefined
          : (url: URL, protocols: readonly string[]) =>
              this.webSocketFactory!(url, protocols);
      socket = await this.client.openSocket(
        request,
        factory as DshWebSocketFactory | undefined,
      );
    } catch (error) {
      ...
      throw new MtmHarnessError(
        error instanceof Error
          ? error.message
          : "Unable to create the conversation stream",
        code,
      );
    }
    return new Promise((resolve, reject) => {
      ...
      socket.onopen = () => {
        ...
        if (this.sessionId === sessionId && this.selectionVersion === version)
          this.update({ connectionStatus: "connected" });
        resolve(socket);
      };
      socket.onmessage = (event) =>
        this.handleSocketMessage(event.data, sessionId, version);
      socket.onerror = () => {
        if (!settled) {
          fail(
            "Unable to connect to the conversation stream",
            "websocket_auth_failed",
          );
        } else if (
          this.sessionId === sessionId &&
  ...
  private handleSocketMessage(
    ...
    const failed =
      ...
      (reason as JsonRecord).kind === "error";
    this.update({
      status: failed
        ? "error"
      ...
      error: failed
        ? "The agent could not complete the conversation."
        : this.snapshot.error,
      sessions: this.snapshot.sessions.map((session) =>
        session.sessionId === sessionId
          ? {
              ...session,
              running: event.type !== "turn/end",
              updatedAt: Date.now(),
            }
          : session,
      ),
  ...
  private handleAuthSnapshot(snapshot: MtmHarnessAuthSnapshot): void {
    const nextPartition =
      snapshot.status === "authenticated"
        ? snapshot.accountPartition
        : undefined;
    const changedAccount =
      nextPartition !== undefined &&
      this.activeAccountPartition !== undefined &&
      nextPartition !== this.activeAccountPartition;
    const signedOut = snapshot.status === "signed-out";
    const hasAccountState =
      this.activeAccountPartition !== undefined ||
      this.sessionId !== undefined ||
      this.snapshot.selectedSessionId !== undefined ||
      this.snapshot.selectedSandboxId !== undefined ||
      this.snapshot.messages.length > 0 ||
      this.snapshot.sandboxes.length > 0 ||
      this.snapshot.workspaces.length > 0 ||
      this.snapshot.sessions.length > 0;
    if ((signedOut || changedAccount) && hasAccountState)
  ...
  private scheduleReconnect(sessionId: string, version: number): void {
    ...
    if (this.reconnectAttempt >= 5) {
      this.update({
        ...
        operation: undefined,
        connectionStatus: "disconnected",
        error: "The conversation stream was disconnected",
      });
      return;
    }
    ...
    this.update({
      ...
      error: "Reconnecting to the conversation stream",
    });
  ...
  private update(patch: Partial<RuntimeSnapshot>): void {
    this.snapshot = {
      ...this.snapshot,
      ...patch,
      messages: patch.messages ?? this.snapshot.messages,
    };
    for (const listener of this.listeners) listener(this.snapshot);
  }
  ...
  private upsertSession(
    ...
    const existing = this.snapshot.sessions.some(
    ...
    const workspaces =
      workspaceId === undefined
        ...
        : this.snapshot.workspaces.map((workspace) =>
            workspace.workspaceId === workspaceId &&
            !workspace.sessionIds.includes(session.sessionId)
              ? {
                  ...workspace,
                  sessionIds: [...workspace.sessionIds, session.sessionId],
                }
              : workspace,
          );
  ...
  private closeSocket(): void {
    ...
    this.socket = undefined;
    if (this.snapshot.connectionStatus !== "disconnected")
      this.update({ connectionStatus: "disconnected" });
    if (socket) this.closeSocketInstance(socket);
  }

  private currentSandboxScope():
    | { sandboxId: string; workspaceId: string }
    | undefined {
    const selected = this.snapshot.sandboxes.find(
      (sandbox) => sandbox.id === this.snapshot.selectedSandboxId,
    );
  ...
  private sessionStorageKey(): string | undefined {
    ...
    const accountPartition =
      this.activeAccountPartition ??
      this.tokenSource?.getAccountPartition() ??
      this.accountPartitionHint;
    return sandboxId === undefined || accountPartition === undefined
      ? undefined
      : this.storageKeyPrefix +
          ":" +
          encodeURIComponent(accountPartition) +
          ":" +
          sandboxId;
  }
  ...
  private clearStoredAccountHints(accountPartition: string | undefined): void {
    ...
    const prefix =
      this.storageKeyPrefix + ":" + encodeURIComponent(accountPartition) + ":";
    try {
