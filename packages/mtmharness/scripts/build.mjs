...
function packageExport(subpath, condition = "default") {
  ...
  const target =
    typeof definition === "string"
      ? definition
      : (definition?.[condition] ?? definition?.default);
  if (typeof target !== "string")
    throw new Error(
      "mtmharness build: package export " +
        subpath +
        " has no " +
        condition +
        " target",
...
function packageField(field) {
  ...
  if (typeof target !== "string")
    throw new Error(
      "mtmharness build: package field " + field + " has no target",
    );
  return resolve(packageRoot, target);
}
...
if (!existsSync(tsc) || !existsSync(vite))
  throw new Error(
    "mtmharness build: local TypeScript and Vite executables are required",
  );

execFileSync(tsc, ["--project", resolve(packageRoot, "tsconfig.json")], {
  cwd: packageRoot,
  stdio: "inherit",
});
...
const embedInputs = clientInputs.filter(
  (input) => input.includes("src/embed/") || input.includes("src\\embed\\"),
);
if (embedInputs.length > 0) {
  throw new Error(
    "mtmharness build: DSH client entry imports embed sources: " +
      embedInputs.join(", "),
  );
}

const clientSource = readFileSync(clientTemp, "utf8");
const indented = clientSource
  .split("\n")
  .map((line) => "    " + line)
  .join("\n");
const artifact = [
...
if (
  !artifact.includes("window.__ModuleLoader__.load") ||
  !artifact.includes('id: "' + packageName + '"')
) {
  throw new Error(
    "mtmharness build: generated client artifact does not have the DSH loader contract",
  );
}
