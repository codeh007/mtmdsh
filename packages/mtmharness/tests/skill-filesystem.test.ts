...
} from "../src/features/coding/manifest.ts";
...
const packageManifest: MtmCodingPackageManifest & {
...
describe("external coding skills", () => {
    ...
    try {
      await ensureSkillPackage(packageManifest, {
        ...
        fetch: async (url) => {
          urls.push(url);
          return new Response(
            url.endsWith("/scripts/run-tool.sh") ? RESOURCE : DOCUMENT,
          );
        },
      });
      expect(urls).toEqual([
        ...
        "https://raw.githubusercontent.com/example/skills/" +
          source.revision +
          "/skills/example-skill/scripts/run-tool.sh",
      ]);
      expect(
        await readFile(join(root, "example-skill", "SKILL.md"), "utf8"),
      ).toBe(DOCUMENT);
      expect(
        await readFile(
          join(root, "example-skill", "scripts", "run-tool.sh"),
          "utf8",
        ),
      ).toBe(RESOURCE);
      ...
      await ensureSkillPackage(packageManifest, {
        root,
        fetch: async () => {
          throw new Error("existing roots must not download");
        },
      });
      expect((await ctx.skills.get("example-skill"))?.content).toContain(
        "User edit.",
      );
      ...
      expect(
        await readFile(join(legacyRoot, "example-skill", "SKILL.md"), "utf8"),
      ).toBe(DOCUMENT);
      expect(
        await readFile(
          join(legacyRoot, "example-skill", "scripts", "run-tool.sh"),
          "utf8",
        ),
      ).toBe(RESOURCE);
      ...
      const invalid = {
        ...
        skills: {
          ...source,
          files: [{ ...source.files[0], sha256: "0".repeat(64) }],
        },
      };
      await expect(
        ensureSkillPackage(invalid, {
          root: invalidRoot,
          fetch: async () => new Response(DOCUMENT),
        }),
      ).rejects.toThrow("checksum mismatch");
      await expect(
        readFile(join(invalidRoot, "example-skill", "SKILL.md"), "utf8"),
      ...
      const unsafe = {
        ...
        skills: {
          ...source,
          files: [{ ...source.files[0], path: "../SKILL.md" }],
        },
      };
      await expect(
        ensureSkillPackage(unsafe, {
          root: unsafeRoot,
          fetch: async () => new Response(DOCUMENT),
        }),
      ).rejects.toThrow("invalid skill metadata");
      await expect(
        readFile(join(unsafeRoot, "example-skill", "SKILL.md"), "utf8"),
      ).rejects.toMatchObject({ code: "ENOENT" });
    } finally {
  ...
  it("cancels an in-flight bundle install without leaving a partial root", async () => {
    ...
    try {
      const pending = ensureSkillPackage(
        { ...packageManifest, id: "cancelled" },
        {
          root,
      ...
      await expect(readFile(root, "utf8")).rejects.toMatchObject({
        code: "ENOENT",
      });
    } finally {
      await rm(temporary, { recursive: true, force: true });
