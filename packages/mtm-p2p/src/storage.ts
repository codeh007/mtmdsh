...
export async function loadStorage(): Promise<P2pStorage> {
  if (typeof indexedDB === "undefined") return createState();
  const value = await get();
  if (value && typeof value === "object" && "identity" in value)
    return normalize(value as P2pStorage);
...
export async function createIdentity(): Promise<{
  ...
  return {
    identity: key.publicKey.raw ? key.publicKey.raw.toString() : "",
...
function createState(): P2pStorage {
  return { identity: crypto.randomUUID(), peers: [] };
}
function normalize(value: P2pStorage): P2pStorage {
  return {
    identity: value.identity,
    privateKey: value.privateKey ? new Uint8Array(value.privateKey) : undefined,
