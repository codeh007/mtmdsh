...
function NodeCard({
  ...
  return (
    <article
      className={
        "mtmcanvas-node" + (selected ? " mtmcanvas-node-selected" : "")
      }
      style={{
    ...
    >
      <button
        type="button"
        className="mtmcanvas-node-title"
        onClick={onSelect}
      ...
      {node.kind === "prompt" ? (
        <p>{node.prompt || "Empty prompt"}</p>
      ) : (
        <div className="mtmcanvas-empty-preview">Image output</div>
      )}
    </article>
  );
}
...
export function CanvasView({
  ...
  function moveNode(node: CanvasNode, event: DragEvent<HTMLElement>): void {
    ...
    actions.moveNode(node.id, {
      x: Math.max(
        0,
        (event.clientX - rect.left - viewport.x) / viewport.k -
          node.size.width / 2,
      ),
      y: Math.max(
        0,
        (event.clientY - rect.top - viewport.y) / viewport.k - 24 / viewport.k,
      ),
    });
  }
  ...
  function zoom(delta: number): void {
    ...
    actions.setViewport({
      ...document.viewport,
      k: Math.min(5, Math.max(0.05, document.viewport.k + delta)),
    });
  }
  ...
  return (
    <section className="mtmcanvas-view" aria-label="MTM Canvas">
      <header className="mtmcanvas-header">
        <div className="mtmcanvas-heading">
          <strong>Canvas</strong>
          <span>{state.name ?? "No file selected"}</span>
        </div>
        <div className="mtmcanvas-header-actions">
          ...
          <span>
            {document === undefined
              ? "--"
              : Math.round(document.viewport.k * 100) + "%"}
          </span>
          <button
            type="button"
            onClick={() => zoom(0.1)}
          ...
          <button
            ...
            disabled={document === undefined || state.version === undefined}
          >
            Save
          </button>
        </div>
        ...
        <aside className="mtmcanvas-files" aria-label="Canvas files">
          ...
          {state.files.map((file) => (
            <button
              ...
              type="button"
              className={
                file.name === state.name ? "mtmcanvas-file-selected" : ""
              }
          ...
          {state.files.length === 0 && !state.loading ? (
            <span>No canvas files yet.</span>
          ) : null}
        </aside>
        <section
          ref={stageRef}
          className="mtmcanvas-stage"
          aria-label="Canvas stage"
        >
          {document === undefined ? (
            <span className="mtmcanvas-stage-empty">
              Create or open a canvas file.
            </span>
          ) : (
            <div
              className="mtmcanvas-layer"
              style={{
                transform:
                  "translate(" +
                  document.viewport.x +
            ...
            >
              {document.connections.map((connection) => {
                const from = document.nodes.find(
                  (node) => node.id === connection.fromNodeId,
                );
                const to = document.nodes.find(
                ...
                return (
                  <div
                    key={connection.id}
                    className="mtmcanvas-connection"
                    style={{
                      left: from.position.x + from.size.width,
                      top: from.position.y + from.size.height / 2,
                      width,
                    }}
                  />
                );
              ...
              {document.nodes.map((node) => (
                <NodeCard
                  ...
                  onDragEnd={(event) => moveNode(node, event)}
                />
              ))}
            </div>
          )}
        ...
        <aside className="mtmcanvas-inspector" aria-label="Canvas inspector">
          ...
          {selected?.kind === "prompt" ? (
            <>
              ...
              <button
                type="button"
                onClick={() => actions.updatePrompt(selected.id, draft)}
              >
                Apply
              </button>
            </>
          ...
          {state.error ? (
            <p className="mtmcanvas-error" role="alert">
              {state.error}
            </p>
          ) : null}
          {state.conflict && state.name !== undefined ? (
            <button
              type="button"
              onClick={() => {
                if (state.name !== undefined) actions.open(state.name);
              }}
            >
              Reload file
            </button>
