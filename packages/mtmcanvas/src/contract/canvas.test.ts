...
describe("Canvas document contract", () => {
  it("round-trips the file document shape", () => {
    ...
    expect(
      validateCanvasDocument(JSON.parse(JSON.stringify(document))),
    ).toEqual(document);
  });
  ...
  it("rejects duplicate nodes and dangling connections", () => {
    ...
    expect(() =>
      validateCanvasDocument({
        ...document,
        connections: [
          { id: "bad", fromNodeId: "missing", toNodeId: "image-1" },
        ],
      }),
    ).toThrow("invalid");
  });
  ...
  it("rejects invalid node sizes", () => {
    ...
    expect(() =>
      validateCanvasDocument({
        ...document,
        nodes: [
          { ...document.nodes[0], size: { width: 0, height: 10 } },
          document.nodes[1],
        ],
      }),
    ).toThrow("size");
  ...
  it("rejects non-string and oversized prompts", () => {
    ...
    expect(() =>
      validateCanvasDocument({
        ...document,
        nodes: [{ ...document.nodes[0], prompt: 42 }, document.nodes[1]],
      }),
    ).toThrow("prompt");
    expect(() =>
      validateCanvasDocument({
        ...document,
        nodes: [
          { ...document.nodes[0], prompt: "x".repeat(20_001) },
          document.nodes[1],
        ],
      }),
    ).toThrow("prompt");
