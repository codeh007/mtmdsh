...
function parseSize(value: unknown, label: string): CanvasSize {
  ...
  if (
    value.width <= 0 ||
    value.height <= 0 ||
    value.width > MAX_NODE_SIZE ||
    value.height > MAX_NODE_SIZE
  )
    throw new Error(label + " is invalid");
...
function parseViewport(value: unknown): CanvasViewport {
  ...
  return {
    x: coordinate(value.x, "canvas viewport.x"),
    y: coordinate(value.y, "canvas viewport.y"),
    k: value.k,
  };
}

export function validateCanvasDocument(value: unknown): CanvasDocument {
  if (!isRecord(value) || value.schemaVersion !== CANVAS_SCHEMA_VERSION)
    throw new Error("canvas document schema version is invalid");
  ...
  if (
    ...
    throw new Error("canvas revision is invalid");
  ...
  if (
    ...
    throw new Error("canvas connections are invalid");
  const updatedAt = value.updatedAt;
  if (!isFiniteNumber(updatedAt) || updatedAt < 0)
    throw new Error("canvas updatedAt is invalid");
  ...
  const nodes = value.nodes.map((raw, index): CanvasNode => {
    ...
    nodeIds.add(id);
    if (raw.kind !== "prompt" && raw.kind !== "image")
      throw new Error("canvas node " + index + " kind is invalid");
    return {
      id,
      kind: raw.kind,
  ...
  const connections = value.connections.map((raw, index): CanvasConnection => {
    if (!isRecord(raw))
      throw new Error("canvas connection " + index + " is invalid");
    const id = boundedText(raw.id, "canvas connection " + index + " id");
    const fromNodeId = boundedText(
      raw.fromNodeId,
      "canvas connection " + index + " fromNodeId",
    );
    const toNodeId = boundedText(
      raw.toNodeId,
      "canvas connection " + index + " toNodeId",
    );
    if (
  ...
  return {
    ...
    canvasId: boundedText(value.canvasId, "canvasId"),
    revision,
    nodes,
    connections,
...
export function createCanvasDocument(
  ...
  return {
    ...
    nodes: [prompt, image],
    connections: [
      { id: "connection-1", fromNodeId: prompt.id, toNodeId: image.id },
    ],
    viewport: { x: 40, y: 30, k: 0.86 },
    updatedAt: now,
  };
}
